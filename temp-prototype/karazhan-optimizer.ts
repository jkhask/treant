...
} from '../src/services/blizzard'
import { config } from 'dotenv'
import * as fs from 'fs'
import * as path from 'path'

// Load environment variables from the treant project
config({ path: path.join(__dirname, '../.env') })

const GEMINI_API_KEY = process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY

if (!GEMINI_API_KEY) {
  console.error('API key (GEMINI_API_KEY or GOOGLE_API_KEY) is not set in the .env file.')
  process.exit(1)
}
  ...
  parameters: {
    ...
    properties: {
      ...
      realmSlug: {
        type: SchemaType.STRING,
        description:
          'The slugified realm name of the character (e.g., crusader-strike, wild-growth).',
      },
    },
    required: ['characterName', 'realmSlug'],
  },
}
...
const getBisList = async (className: string, specName: string) => {
  const dataPath = path.join(__dirname, 'bis_data.json')
...
const getCharacterGear = async (characterName: string, realmSlug: string) => {
  try {
    const creds = await getBlizzardCredentials()
    if (!creds) {
      // Fallback for local testing without AWS Secret Manager access
      return {
        error:
    ...
    // Simplify the equipment response to just the item names for the AI to easily parse
    ...
    // Return a graceful fallback payload for the LLM
    return {
...
`
  ...
  console.log('Sending prompt to Gemini. This may take a minute or two as it uses tools...')
